...
ifeq ($(BUILDARCH), x86)
    ...
    XBUILD_SDKUCRTLIBDIR=$(XBUILD_WINKITROOT)/Lib/$(XBUILD_SDKVER)/ucrt/x86
else
    XBUILD_SDKBINDIR=$(XBUILD_WINKITROOT)/bin/$(XBUILD_SDKVER)/x64
    XBUILD_SDKLIBDIR=$(XBUILD_WINKITROOT)/Lib/$(XBUILD_SDKVER)/um/x64
    XBUILD_SDKUCRTLIBDIR=$(XBUILD_WINKITROOT)/Lib/$(XBUILD_SDKVER)/ucrt/x64
endif
...
TOOL_SIGNTOOL=$(XBUILD_SDKBINDIR)/signtool.exe
...
ifeq ($(TARGETMODE),kernel)
    ...
    BUILD_LIBDIRS += -LIBPATH:"$(XBUILD_VCLIBDIR)" -LIBPATH:"$(XBUILD_VCATLMFC_LIBDIR)" -LIBPATH:"$(XBUILD_SDKLIBDIR)" -LIBPATH:"$(XBUILD_SDKUCRTLIBDIR)"
endif

BUILD_LFLAGS += $(BUILD_LIBDIRS)
BUILD_SLFLAGS += $(BUILD_LIBDIRS)
...
TARGET_LFLAGS_INCREMENTAL=false
ifneq ($(TARGETMODE),kernel)
    ifeq ($(BUILDTYPE),debug)
        TARGET_LFLAGS_INCREMENTAL=true
    endif
endif
ifeq ($(TARGET_LFLAGS_INCREMENTAL),true)
    BUILD_LFLAGS += -INCREMENTAL
else
    BUILD_LFLAGS += -INCREMENTAL:NO
endif

ifeq ($(TARGET_LFLAGS_IGNOREDEFULT),true)
    BUILD_LFLAGS  += -NODEFAULTLIB
endif

ifneq ($(TARGET_LFLAGS_DELAYLOAD_DLLS),)
    BUILD_LFLAGS  += $(addprefix -DELAYLOAD:,$(TARGET_LFLAGS_DELAYLOAD_DLLS))
endif

ifneq ($(TARGET_LFLAGS_FUNCTION_ORDER_FILE),)
    BUILD_LFLAGS += -ORDER:@"$(TARGET_LFLAGS_FUNCTION_ORDER_FILE)"
endif

ifneq ($(TARGETMODE),kernel)
    ifeq ($(BUILDTYPE),release)
        ifeq ($(TARGET_LFLAGS_FASTLTCG),)
            TARGET_LFLAGS_FASTLTCG=true
        endif
        ifeq ($(TARGET_LFLAGS_FASTLTCG),true)
            BUILD_LFLAGS += -LTCG:incremental
        else
            BUILD_LFLAGS += -LTCG
        endif
    endif
...
ifeq ($(TARGETMODE), kernel)
    ...
    BUILD_LFLAGS += -kernel -MANIFEST:NO -Driver -PDB:$(BUILD_INTDIR)/$(TARGETNAME).pdb -VERSION:"$(TARGET_SUBSYSVER)" -WX -DEBUG -RELEASE \
	            -SUBSYSTEM:$(TARGET_SUBSYS),$(TARGET_SUBSYSVER) -ERRORREPORT:PROMPT -MERGE:"_TEXT=.text;_PAGE=PAGE" -NOLOGO -NODEFAULTLIB -SECTION:"INIT,d" -IGNORE:4078
    BUILD_LIBS=fltmgr.lib BufferOverflowK.lib ntoskrnl.lib hal.lib wmilib.lib Ntstrsafe.lib
    BUILD_SLFLAGS += -NOLOGO -DEBUG
    ifeq ($(BUILDARCH), x64)
    ...
    BUILD_LFLAGS += -NOLOGO -DYNAMICBASE -NXCOMPAT -ERRORREPORT:PROMPT -PDB:"$(BUILD_INTDIR)/$(TARGETNAME).pdb" -TLBID:1 -SUBSYSTEM:$(TARGET_SUBSYS) -DEBUG \
    ...
    BUILD_SLFLAGS += -NOLOGO -LTCG -SUBSYSTEM:WINDOWS -DEBUG
    ifeq ($(TARGETTYPE),dll)
        BUILD_LFLAGS += -DLL -IMPLIB:"$(BUILD_INTDIR)/$(TARGETNAME).lib"
    endif
    ifneq ($(TARGET_LFLAGS_ENTRYPOINT),)
        BUILD_LFLAGS  += -ENTRY:"$(TARGET_LFLAGS_ENTRYPOINT)"
    endif
    ifeq ($(TARGET_LFLAGS_SETCHECKSUM),true)
        BUILD_LFLAGS  += -RELEASE
    endif
    ifeq ($(TARGET_LFLAGS_PROFILE),true)
        BUILD_LFLAGS  += -PROFILE
    endif
endif

# Default Linked Libraried
ifneq ($(TARGETLIBS),)
    BUILD_LIBS+=$(TARGETLIBS)
endif
BUILD_LFLAGS+=$(BUILD_LIBS)
